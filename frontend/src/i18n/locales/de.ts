export const de = {
  common: {
    settings: "Einstellungen",
    download: "Herunterladen",
    loading: "Laden...",
    save: "Speichern",
    saved: "Erfolgreich gespeichert",
    cancel: "Abbrechen",
    error: "Fehler aufgetreten",
    verify: "Überprüfen",
    artist: "Künstler",
    create: "Erstellen",
    disconnect: "Trennen",
    tracks: "Titel",
    view_profile: "Profil ansehen",
  },
  sidebar: {
    home: "Startseite",
    search: "Suche",
    watchlist: "Beobachtungsliste",
    library: "Meine Bibliothek",
    downloads: "Downloads",
    settings: "Einstellungen",
    discover: "Entdecken",
    logs: "Protokolle",
  },
  logs: {
    title: "Systemprotokolle",
    subtitle: "Backend-Protokolle zur Fehlerbehebung.",
    autoRefresh: "Automatisch aktualisieren",
    clear: "Löschen",
    refresh: "Aktualisieren",
    lines: "Zeilen",
    download: "Protokolle herunterladen",
    showErrorsOnly: "Nur Fehler",
  },
  dashboard: {
    title: "Dashboard",
    subtitle: "Übersicht über deine Musiksammlung und Downloads.",
    searchPlaceholder: "URL einfügen (YouTube, Spotify...) oder nach Musik suchen...",

    stats: {
      totalDownloads: "Gesamte Downloads",
      tracksLibrary: "Titel in Bibliothek",
      pendingQueue: "Warteschlange",
      storageFree: "Freier Speicher",
    },
    recentActivity: "Letzte Aktivität",
    noActivity: "Keine letzte Aktivität",
    quickActions: "Schnellaktionen",
    quickLinks: {
      library: "Bibliothek",
      watchlist: "Beobachtungsliste",
      queue: "Warteschlange",
      settings: "Einstellungen",
    },
    actions: {
      addDownload: "Neuen Download hinzufügen",
      importPlaylist: "Playlist importieren",
      viewWatchlist: "Beobachtungsliste ansehen",
    },
    activeDownload: {
      downloading: "Wird heruntergeladen...",
      pending: "Ausstehend...",
      completed: "Abgeschlossen",
      noActive: "Keine aktiven Downloads",
      startDownloading: "Starte einen Download, um hier den Fortschritt zu sehen",
    },
  },
  footer: {
    rights: "Alle Rechte vorbehalten.",
  },
  notifications: {
    title: "Benachrichtigungen",
    markAllRead: "Alle als gelesen markieren",
    clearAll: "Alles löschen",
    empty: "Keine Benachrichtigungen",
  },
  usermenu: {
    currentAccount: "Aktuelles Konto",
    switchAccount: "Konto wechseln",
    addAccount: "Weiteres Konto hinzufügen",
    settings: "Einstellungen",
    logout: "Abmelden",
    removeAccount: "Konto entfernen",
  },
  settings: {
    language: "Sprache",
    audioQuality: "Audioqualität",
    theme: "Design",
    general: "Allgemein",
    account: "Konto",
    appearance: "Aussehen",
    files: "Dateien & Speicher",
    integrations: "Integrationen",
    registration: "Neue Registrierungen zulassen",
    registrationDesc: "Wenn deaktiviert, kann niemand ein neues Konto erstellen.",
    subtitle: "Verwalten Sie Ihre API-Schlüssel und Anwendungseinstellungen.",
    preview: "Vorschau:",
    availableTags: "Verfügbare Tags:",
    profileInfo: "Profilinformationen",
    username: "Benutzername",
    avatarUrl: "Avatar-URL",
    saveProfile: "Profil speichern",
    changePassword: "Passwort ändern",
    currentPassword: "Aktuelles Passwort",
    newPassword: "Neues Passwort",
    updatePassword: "Passwort aktualisieren",
    messages: {
      profileUpdated: "Profil erfolgreich aktualisiert",
      passwordUpdated: "Passwort erfolgreich aktualisiert",
      avatarUpdated: "Avatar erfolgreich aktualisiert",
      updateError: "Aktualisierung fehlgeschlagen",
      uploadError: "Upload fehlgeschlagen",
    },
    themes: {
      dark: "Audiovault Standard",
      midnight: "Mitternachtsblau",
      ocean: "Tiefer Ozean",
      forest: "Dunkler Wald",
      sunset: "Sonnenuntergang",
      neon: "Cyber Neon",
      classic: "Klassisch",
    },
    downloadPath: "Download-Pfad",
    filenameSchema: "Dateinamenschema",
    maxDownloads: "Max. parallele Downloads",
    spotify: "Spotify",
    youtube: "YouTube",
    clientId: "Client-ID",
    clientSecret: "Client-Secret",
    apiKey: "API-Schlüssel",
  },
  quality: {
    low: "Niedrig (128kbps)",
    normal: "Normal (192kbps)",
    high: "Hoch (320kbps)",
    lossless: "Verlustfrei (FLAC)",
  },
  filters: {
    allTypes: "Alle Typen",
    artists: "Künstler",
    playlists: "Playlists",
    tracks: "Titel",
    allSources: "Alle Quellen",
    search: "Suchen",
  },
  search: {
    title: "Musik suchen",
    searching: "Suche...",
    subtitle:
      "Finden Sie Ihre Lieblingstitel von Spotify, YouTube, Deezer, Apple Music, Tidal, Amazon Music und SoundCloud",
    placeholder: "Was möchten Sie hören?",
    noResults: "Keine Ergebnisse gefunden. Versuchen Sie eine andere Suche.",
    loadMore: "Mehr laden",
    headers: {
      tracks: "Titel",
      artists: "Künstler",
      playlists: "Playlists",
    },
  },
  library: {
    title: "Meine Bibliothek",
    subtitle: "Verwalten Sie Ihre heruntergeladene Musik.",
    newPlaylist: "Neue Playlist",
    rescan: "Bibliothek scannen",
    allTracks: "Alle Titel",
    uncategorized: "Unkategorisiert",
    playlistDeleted: "Playlist gelöscht",
    trackDeleted: "Titel gelöscht",
    rescanComplete: "Scan abgeschlossen",
  },
  watchlist: {
    title: "Beobachtungsliste",
    subtitle: "Verfolgen Sie Künstler und Playlists für neue Veröffentlichungen.",
    autoDownloadOn: "Auto-Download: EIN",
    autoDownloadOff: "Auto-Download: AUS",
    syncDeletions: "Löschungen synchronisieren",
    syncAllDeletions: "Alle Löschungen synchronisieren",
    autoSyncDeletionsOn: "Auto-Sync Löschungen: EIN",
    autoSyncDeletionsOff: "Auto-Sync Löschungen: AUS",
    syncAllSuccess: "Sync abgeschlossen. Entfernt: {{removed}}, Übersprungen: {{skipped}}",
    removeFromWatchlist: "Von Beobachtungsliste entfernen",
    newItems: "neu",
    addToWatchlist: "Zur Beobachtungsliste hinzufügen",
    viewDetails: "Details anzeigen",
  },
  recommendations: {
    tabs: {
      tracks: "Titel",
      artists: "Künstler",
      playlists: "Playlists",
    },
    title: "Musik entdecken",
    subtitle: "Personalisierte Auswahl basierend auf Ihren Hörgewohnheiten",
    refreshed: "Neue Empfehlungen geladen",
    refreshError: "Empfehlungen konnten nicht aktualisiert werden",
  },
  listening: {
    connect: "Verbinden",
    connectedAs: "Verbunden als {name}",
    tokenPlaceholder: "Token einfügen",
    tokenError: "Ungültiger Token",
    recommendationSource: "Empfehlungsquelle",
    auto: "Automatisch",
    connectPrompt: "Verbinde einen Hördienst, um zu starten",
    connectHint: "Verbinde Last.fm oder ListenBrainz, damit wir deinen Hörverlauf analysieren können.",
    getToken: "Wo bekomme ich einen ListenBrainz-Token?",
  },
  lastfm: {
    connect: "Mit Last.fm verbinden",
    connected: "Erfolgreich mit Last.fm verbunden!",
    error: "Verbindung zu Last.fm fehlgeschlagen",
    scrobbling: "Scrobbling aktiviert",
    profile: {
      error: "Profil konnte nicht geladen werden",
      artists: "Künstler",
      tracks: "Titel",
      albums: "Alben",
      friends: "Freunde",
    },
  },
  artist: {
    not_found: "Künstler nicht in der Bibliothek gefunden",
  },
  player: {
    play: "Abspielen",
  },
  playlist: {
    addButton: "Zur Playlist hinzufügen",
    modal: {
      title: "Zur Playlist hinzufügen",
    },
    fetch: {
      error: "Playlists konnten nicht geladen werden",
    },
    create: {
      error: "Playlist konnte nicht erstellt werden",
      new: "Neue Playlist erstellen",
      placeholder: "Playlist-Name",
    },
    add: {
      success: "{{count}} Titel zur Playlist hinzugefügt",
      duplicates: "{{count}} Titel waren bereits in der Playlist",
      error: "Titel konnten nicht hinzugefügt werden",
    },
  },
};
