...
export default function ArtistCard({ artist }: Readonly<ArtistCardProps>) {
  ...
  const handleClick = () => {
    navigate(`/artist/${artist.id}`, { state: { source: artist.source } });
  };
  ...
  return (
    ...
    >
      ...
      <h3 className="font-bold text-white text-center truncate w-full group-hover:text-primary transition-colors">
        {artist.name}
      </h3>
