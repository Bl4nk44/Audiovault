...
export default function DownloadItem({ item }: DownloadItemProps) {
    ...
    const getStatusBadge = () => {
        switch (item.status) {
            case 'completed':
                return (
                    <span className="px-2 py-1 rounded-full bg-green-500/20 text-green-400 text-xs font-medium border border-green-500/20 flex items-center gap-1">
                        <CheckCircle size={12} /> Completed
                    </span>
                )
            case 'failed':
                return (
                    <span className="px-2 py-1 rounded-full bg-red-500/20 text-red-400 text-xs font-medium border border-red-500/20 flex items-center gap-1">
            ...
            default:
                return (
                    <span className="px-2 py-1 rounded-full bg-gray-500/20 text-gray-400 text-xs font-medium border border-gray-500/20">
                        Pending
                    </span>
                )
        }
    ...
    return (
        <motion.div
            ...
            onClick={handlePlay}
            className={cn(
                "group relative bg-white/5 backdrop-blur-md rounded-2xl p-4 flex items-center gap-5 transition-all shadow-lg overflow-hidden",
                isDownloading ? "border-2 border-primary/50 shadow-[0_0_20px_rgba(34,197,94,0.15)] bg-white/10" : "border border-white/5",
                item.status === 'completed' ? "cursor-pointer hover:bg-white/10 hover:scale-[1.01]" : ""
            )}
        >
            ...
            {/* Cover Image */}
            <div className={cn(
                "w-16 h-16 bg-black/40 rounded-xl overflow-hidden flex-shrink-0 border shadow-md relative group-hover:scale-105 transition-transform duration-300 z-10",
                ...
                {isDownloading && (
                    <div className="relative h-2 w-full bg-black/20 rounded-full overflow-hidden mt-3 border border-white/5">
                        <motion.div
                            ...
                            className="absolute top-0 left-0 h-full bg-gradient-to-r from-primary/80 to-primary shadow-[0_0_10px_rgba(34,197,94,0.5)]"
                        />
                    </div>
                )}
            ...
            <div className="flex items-center gap-2 flex-shrink-0 z-10">
                {item.status === 'completed' && (
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all">
