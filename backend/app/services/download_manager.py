...
class DownloadManager:
    def __init__(self):
        ...
        self.semaphore = asyncio.Semaphore(settings.MAX_PARALLEL_DOWNLOADS)
    ...
    async def process_queue(self):
        logger.info(f"Worker started with {settings.MAX_PARALLEL_DOWNLOADS} concurrent slots")
        while True:
            ...
            # We strictly don't await it here, to allow next iteration to pick up next job if slot available
            asyncio.create_task(self._wrapped_process_download(download_id))

    async def _wrapped_process_download(self, download_id):
        try:
            current_task = asyncio.current_task()
            self.active_tasks[download_id] = current_task
            
            await self.process_download(download_id)
            
        except asyncio.CancelledError:
            logger.info(f"Download task {download_id} cancelled")
            # process_download handles logging/DB update for error usually, but cancel might interrupt it
    ...
    async def add_download(
        self, db: AsyncSession, user_id: str, download_data: "DownloadCreate"
    ) -> Download:
        download = Download(
            user_id=user_id,
    ...
    async def _handle_completion(
        self, db, download, final_filename_container, output_template
    ):
        ...
        self._set_download_file_path(
            download, final_filename_container, output_template
        )
        self._fix_filename_artifacts(download)

        logger.info(
            f"💾 Saving file for user '{download.user.username}' to: {download.file_path}"
        )
        await db.commit()
    ...
    def _set_download_file_path(
        self, download, final_filename_container, output_template
    ):
...
=======
            ...
            if not download_path:
                download_path = os.path.join(
                    settings.DOWNLOAD_DIR, download.user.username
                )
...
>>>>>>> temp/release-prep
            ...
            if filename.startswith("NA -"):
                ...
                if len(new_filename) > 0:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Renamed file to remove NA prefix: {filename} -> {new_filename}"
                        )
                    except Exception as e:
    ...
    async def _handle_error(self, db, download, e):
        if isinstance(e, DownloadPausedError) or str(e) == "DOWNLOAD_PAUSED":
            ...
            await db.commit()
            await socket_manager.emit(
                "download:paused", {"download_id": str(download.id)}
            )
            return
        ...
        await socket_manager.emit(
            "download:error", {"download_id": str(download.id), "error": str(e)}
        )
    ...
    async def process_download(self, download_id: str):
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                progress_hook = self._create_progress_hook(
                    download_id, download, loop, final_filename_container
                )
                ydl_opts, output_template = self._get_ydl_options(
                ...
                if url:
                    ...
                    await self._handle_completion(
                        db, download, final_filename_container, output_template
                    )
                else:
    ...
    async def _notify_processing(self, download):
        await socket_manager.emit(
            "download:processing",
            {"download_id": str(download.id), "status": "processing"},
        )

    def _create_progress_hook(
        self, download_id, download, loop, final_filename_container
    ):
        def progress_hook(d):
            if d["status"] == "downloading":
                ...
                try:
                    logger.info(
                        f"Download finished: {d['filename']}. Starting conversion/processing..."
                    )

                    # Update status to processing in DB
                    asyncio.run_coroutine_threadsafe(
                        self._set_processing_status(download_id), loop
                    )
    ...
    def _handle_progress_update(self, d, download_id, download, loop):
        try:
                ...
                socket_manager.emit(
                    "download:progress",
                    {
                        ...
                        "track": {
                            "title": download.track.title,
                            "artist": download.track.artist,
                            "image_url": download.track.metadata_content.get(
                                "image_url"
                            )
                            if download.track.metadata_content
                            else None,
            ...
            if int(progress) % 5 == 0:
                asyncio.run_coroutine_threadsafe(
                    self.update_progress_db(download_id, progress), loop
                )
        except Exception as e:
...
>>>>>>> temp/release-prep
    ...
    async def cancel_download(self, db: AsyncSession, download_id: str):
        ...
        if download:
            ...
            await socket_manager.emit(
                "download:cancelled", {"download_id": download_id}
            )
...
>>>>>>> temp/release-prep
        ...
        filename_schema = download.user.preferences.get(
            "filename_schema", "{artist} - {title}"
        )

        logger.info(
            f"Processing download {download.id} with schema: '{filename_schema}'"
        )
        ...
        if not output_template or "%" not in output_template:
            logger.warning(
                f"Output template '{output_template}' invalid or missing tags. Fallback to default."
            )
...
>>>>>>> temp/release-prep
    ...
    async def _resolve_url(self, db: AsyncSession, download: Download) -> str:
        ...
        instruction = fallback_service.get_fallback_instruction(
            download.source, attempt, track_info
        )
        logger.info(
            f"Fallback instruction for {download.source} (Attempt {attempt}): {instruction}"
        )
        ...
        if resp_type == "yt_search":
            ...
            if track_info and track_info.source_url:
                ...
                url = (
                    meta.get("source_url") or track_info.source_url
                )  # track model has source_url column? No, generic
    ...
    async def update_playlist_m3u(
        self, db: AsyncSession, user_id: str, playlist_name: str
    ):
        try:
...
=======
            ...
            if not download_path:
                download_path = os.path.join(
                    settings.DOWNLOAD_DIR, downloads[0].user.username
                )
...
>>>>>>> temp/release-prep
    ...
    async def delete_playlist(
        self, db: AsyncSession, user_id: str, source: str, playlist_name: str
    ):
        """Delete an entire playlist, including files and DB records."""
        try:
            downloads = await self._get_playlist_downloads(
                db, user_id, source, playlist_name
            )

            if not downloads:
                logger.info(
                    f"No downloads found for playlist {playlist_name} ({source})"
                )
                return
    ...
    async def _get_playlist_downloads(
        self, db: AsyncSession, user_id, source, playlist_name
    ):
        result = await db.execute(
            select(Download).where(
                Download.user_id == user_id,
    ...
    async def retry_failed_downloads(self, db: AsyncSession, max_retries: int = 3):
        """
        Automatically retry downloads that failed due to transient errors.
        """
        try:
            # Find downloads that failed, have retry_count < max, and are not paused
            # We could also check updated_at timestamp to ensure we don't retry too fast,
            # but for now let's rely on the scheduler interval (e.g. 5 min).
            result = await db.execute(
                select(Download).where(
                    Download.status == "failed",
            ...
            count = 0
            for download in downloads:
                logger.info(f"Auto-retrying download {download.id} (Attempt {download.retry_count or 0} + 1)")
                download.status = "pending"
                download.error_message = None
                await self.queue.put(download.id)
                count += 1
            ...
            if count > 0:
                await db.commit()
                logger.info(f"Auto-queued {count} failed downloads for retry")
                await self.start_worker()
                
            return count
        except Exception as e:
            logger.error(f"Error in auto-retry logic: {e}")
            return 0
