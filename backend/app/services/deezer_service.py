...
class DeezerService:
    ...
    async def search(self, query: str, limit: int = 20, offset: int = 0) -> List[Dict[str, Any]]:
        ...
        if "deezer.page.link" in query or "deezer.com" in query: # check generic deezer too just in case of weird redirects
        ...
        url_match = re.search(r'(?:https?://)?(?:www\.)?deezer\.com/(?:\w{2}/)?(track|album|playlist)/(\d+)', query)
        if url_match:
    ...
    async def get_artist_details(self, artist_id: str) -> Optional[Dict[str, Any]]:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(f"{self.BASE_URL}/artist/{artist_id}") as response:
                ...
                if 'error' in artist: return None
