...
class LibraryScannerService:
    ...
    def _validate_scan_path(self, scan_path: Optional[str]) -> str:
        if scan_path:
            ...
            if common != self.base_dir:
                raise ValueError(
                    "Access denied: Cannot scan directories outside of download library."
                )
            return target_path
    ...
    async def _get_known_paths(self, db: AsyncSession, user_id: str) -> Set[str]:
        result = await db.execute(
            select(Download.file_path).where(Download.user_id == user_id)
        )
        known_paths = set()
    ...
    def _infer_source_info(self, full_path: str, root_dir: str) -> tuple[str, str]:
        ...
        try:
            ...
            logger.debug(
                f"Failed to infer source info for {full_path}: {e}"
            )  # Fallback to default

        return source, playlist_name

    async def scan_directory(
        self, db: AsyncSession, user_id: str, scan_path: Optional[str] = None
    ) -> dict:
        try:
