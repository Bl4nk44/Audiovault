...
class YouTubeService(BaseMusicService):
    ...
    def search(
        self, query: str, limit: int = 20, type: str = "song"
    ) -> List[Dict[str, Any]]:
    ...
    def _search_keywords(
        self, query: str, limit: int, type: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if "tracks" in playlist:
                for item in playlist["tracks"]:
                    tracks.append(
                        self._format_track(item, album_name=playlist.get("title"))
                    )
