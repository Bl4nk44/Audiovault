...
async def test_validate_url_private_ip():
    ...
    with patch("socket.getaddrinfo", return_value=[(socket.AF_INET, socket.SOCK_STREAM, 0, "", ("192.168.1.1", 80))]):
...
async def test_validate_url_success():
    ...
    with patch(
        "socket.getaddrinfo", return_value=[(socket.AF_INET, socket.SOCK_STREAM, 0, "", ("35.186.224.25", 443))]
    ):
...
async def test_resolve_redirects_success():
    ...
    with patch(
        ...
        with patch("aiohttp.ClientSession") as MockSession:
            ...
            _ = await resolve_redirects("https://open.spotify.com/playlist")
