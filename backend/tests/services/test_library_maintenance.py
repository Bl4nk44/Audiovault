...
async def test_update_download_item_rename(db_session: AsyncSession):
    ...
    with patch("os.path.exists", return_value=True), \
        ...
        await library_maintenance_service.update_download_item(db_session, str(user_id), str(dl.id), {"filename": "new.mp3", "title": "New"})
...
async def test_update_download_item_rename_fail(db_session: AsyncSession):
    ...
    db_session.add(dl)
    await db_session.commit()
    with patch("app.core.dependencies.get_db"), \
        ...
        with pytest.raises(ValueError, match="Failed to rename file"):
            await library_maintenance_service.update_download_item(db_session, str(user_id), str(dl.id), {"filename": "new.mp3"})
...
async def test_update_download_item_all_metadata(db_session: AsyncSession):
    ...
    await library_maintenance_service.update_download_item(db_session, str(user_id), str(dl.id), {"artist": "NewA", "album": "NewAlb"})
