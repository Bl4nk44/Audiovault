...
async def test_progress_hook_finished(download_manager, mock_download):
    ...
    with patch.object(download_manager, "_set_processing_status", new_callable=AsyncMock) as mock_set_status:
...
async def test_handle_progress_update(download_manager, mock_download):
    ...
    with patch("app.services.download_manager.socket_manager.emit", new_callable=AsyncMock) as mock_emit:
        with patch.object(download_manager, "update_progress_db", new_callable=AsyncMock) as mock_db_update:
            with patch("asyncio.run_coroutine_threadsafe") as mock_run:
