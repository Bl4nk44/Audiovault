...
async def test_handle_scan_file_non_audio(scanner, db_session, admin_user):
    result = await scanner._handle_scan_file(
        db_session, str(admin_user.id), "/music/readme.txt", "readme.txt", "/music", set()
    )
...
async def test_handle_scan_file_already_known(scanner, db_session, admin_user):
    ...
    result = await scanner._handle_scan_file(
        db_session, str(admin_user.id), file_path, "known.mp3", "/music", known_paths
    )
    assert result is False
...
async def test_cleanup_orphans(scanner, db_session):
    ...
    dl = Download(
        ...
        track_id=track.id,
        user_id=uuid.uuid4(),
        file_path="/non/existent/orphan.mp3",
        status="completed",
    )
    db_session.add(dl)
    await db_session.commit()
