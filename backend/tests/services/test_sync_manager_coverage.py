...
async def test_analyze_watchlist_high_deletion_warning(sync_mgr):
    ...
    with patch.object(sync_mgr, "_fetch_remote_tracks", return_value=[
        {"id": "spotify_0"}, {"id": "spotify_1"}, {"id": "spotify_2"}
    ]):
...
async def test_execute_sync_success(sync_mgr):
    ...
    token = "valid_token"
    sync_mgr._pending_reports[token] = {
    ...
    with patch.object(sync_mgr, "_soft_delete_file", return_value=True), \
