...
async def test_dm_handle_completion_with_mp3_format(download_manager, mock_download):
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("app.services.library_scanner.library_scanner_service._parse_audio_metadata_sync",
               return_value=("Title MP3", "Artist MP3", "Album MP3", "Pop", 180000)), \
         patch("app.services.library_scanner.library_scanner_service.resolve_artist_and_album",
               new_callable=AsyncMock, return_value=("artist_uuid", "album_uuid")):
...
async def test_dm_handle_completion_with_flac_format(download_manager, mock_download):
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("app.services.library_scanner.library_scanner_service.resolve_artist_and_album",
               new_callable=AsyncMock, return_value=("artist_id", "album_id")):
...
async def test_dm_delete_playlist_full_flow(download_manager):
    ...
    with patch.object(download_manager, "_get_playlist_downloads", 
                     new_callable=AsyncMock, return_value=mock_downloads), \
...
async def test_dm_resolve_url_youtube_search(download_manager, mock_download):
    ...
    with patch.object(download_manager, "get_track_info", 
                     new_callable=AsyncMock, return_value=mock_track), \
         patch("app.services.download_manager.fallback_service.get_fallback_instruction",
...
async def test_dm_resolve_url_soundcloud_search(download_manager, mock_download):
    ...
    with patch.object(download_manager, "get_track_info", new_callable=AsyncMock, return_value=None), \
         patch("app.services.download_manager.fallback_service.get_fallback_instruction",
...
async def test_dm_update_progress_db(download_manager):
    ...
    with patch("app.services.download_manager.AsyncSessionLocal") as mock_session:
        ...
        assert mock_download.progress == 50.0
        db_mock.commit.assert_called_once()
...
def test_dm_get_ydl_options_high_quality(download_manager, mock_download):
    ...
    with patch("app.services.download_manager.settings") as mock_settings:
        ...
        ydl_opts, template = download_manager._get_ydl_options(mock_download, lambda x: None)
        
        assert ydl_opts["format"] == "bestaudio/best"
        assert any(pp.get("preferredquality") == "320" for pp in ydl_opts["postprocessors"] 
...
def test_dm_get_ydl_options_with_playlist_tag(download_manager, mock_download):
    ...
    mock_download.user.preferences = {
        "quality": "high",
        "filename_schema": "{playlist}/{artist} - {title}"
    }
...
async def test_dm_update_playlist_m3u(download_manager):
    ...
    with patch("aiofiles.open", new_callable=MagicMock) as mock_aiofiles, \
        ...
        with patch("app.models.playlist.Playlist"), \
             patch("app.models.playlist.PlaylistTrack"):
...
async def test_dm_process_with_semaphore_invalid_uuid(download_manager):
    ...
    # Queue should be marked done (no hang)
    assert True
...
async def test_dm_execute_download_cache_miss_with_entries(download_manager):
    ...
    with patch("app.services.download_manager.cache_manager.get", 
              new_callable=AsyncMock, return_value=None) as mock_get, \
         patch("app.services.download_manager.cache_manager.set",
...
async def test_dm_execute_download_cache_hit(download_manager):
    ...
    with patch("app.services.download_manager.cache_manager.get",
              new_callable=AsyncMock, return_value="https://cached.url") as mock_get, \
         patch("yt_dlp.YoutubeDL") as mock_ydl:
