...
def spotify_service():
    ...
    with patch("app.services.spotify_service.spotipy.Spotify"), \
...
def test_spotify_get_playlist_tracks_null_track(spotify_service):
    ...
    spotify_service.client.playlist_tracks.return_value = {
        "items": [
            {"track": None},
            {"track": {"id": "valid", "name": "Valid", "artists": [{"name": "A", "id": "a1"}],
                      "album": {"name": "AL", "images": []}, "duration_ms": 100}}
        ],
        "next": None
    }
...
def test_spotify_get_playlist_details(spotify_service):
    ...
    mock_playlist = {
        ...
        "tracks": {
            "total": 2,
            "items": [
                {"track": {"id": "t1", "name": "T1", "artists": [{"name": "A", "id": "a1"}],
                          "album": {"name": "AL", "images": []}, "duration_ms": 100}},
            ],
            "next": None
        }
...
def test_spotify_get_playlist_details_pagination(spotify_service):
    ...
    mock_playlist = {
        ...
        "tracks": {
            "total": 150,
            "items": [{"track": {"id": f"t{i}", "name": f"T{i}", "artists": [{"name": "A", "id": "a1"}],
                               "album": {"name": "AL", "images": []}, "duration_ms": 100}} for i in range(100)],
            "next": "page2"
        }
    }
    spotify_service.client.playlist.return_value = mock_playlist
    spotify_service.client.next.return_value = {
        "items": [{"track": {"id": "t100", "name": "T100", "artists": [{"name": "A", "id": "a1"}],
                            "album": {"name": "AL", "images": []}, "duration_ms": 100}}],
        "next": None
    }
...
def test_spotify_get_artist_details_full(spotify_service):
    ...
    mock_top_tracks = {
        "tracks": [
            {"id": "t1", "name": "Top 1", "artists": [{"name": "Full Artist", "id": "artist1"}],
             "album": {"name": "AL", "images": []}, "duration_ms": 200000}
        ]
    }
    mock_albums = {
        "items": [
            {"id": "alb1", "name": "Album 1", "images": [{"url": "http://alb.img"}],
             "release_date": "2024-01-01", "total_tracks": 12, "type": "album", "album_type": "album"}
        ],
        "next": None
    }
...
def test_spotify_get_artist_details_dedupe_albums(spotify_service):
    ...
    mock_albums = {
        "items": [
            {"id": "alb1", "name": "Same Album", "images": [], "release_date": "2024", "total_tracks": 10, "type": "album"},
            {"id": "alb2", "name": "Same Album", "images": [], "release_date": "2024", "total_tracks": 10, "type": "album"},
            {"id": "alb3", "name": "Other Album", "images": [], "release_date": "2023", "total_tracks": 8, "type": "album"},
        ],
        "next": None
    }
...
def test_spotify_get_artist_top_tracks(spotify_service):
    ...
    mock_tracks = {
        "tracks": [
            {"id": "t1", "name": "Hit 1", "artists": [{"name": "A", "id": "a1"}],
             "album": {"name": "AL", "images": []}, "duration_ms": 180000},
            {"id": "t2", "name": "Hit 2", "artists": [{"name": "A", "id": "a1"}],
             "album": {"name": "AL", "images": []}, "duration_ms": 200000}
        ]
    }
...
def test_spotify_get_artist_albums_pagination(spotify_service):
    ...
    page1 = {
        "items": [{"id": "alb1", "name": "Album 1"}],
        "next": "page2"
    }
    page2 = {
        "items": [{"id": "alb2", "name": "Album 2"}],
        "next": None
    }
...
def test_spotify_get_album_tracks_pagination(spotify_service):
    ...
    page1 = {
        "items": [{"id": f"t{i}", "name": f"T{i}", "artists": [{"name": "A", "id": "a1"}], "duration_ms": 100} for i in range(50)],
        "next": "page2"
    }
    page2 = {
