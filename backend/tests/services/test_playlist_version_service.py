...
async def test_create_snapshot_empty_playlist(db_session, sample_playlist, admin_user):
    ...
    version = await playlist_version_service.create_snapshot(
        db_session, sample_playlist, "CREATE", admin_user.id
    )
...
async def test_create_snapshot_with_tracks(db_session, sample_playlist, sample_track, admin_user):
    ...
    sample_playlist.tracks.append(PlaylistTrack(
    ...
    version = await playlist_version_service.create_snapshot(
        db_session, sample_playlist, "ADD_TRACK", admin_user.id
    )
...
async def test_rollback_reordering(db_session, sample_playlist, admin_user):
    ...
    await db_session.refresh(sample_playlist)
    result = await db_session.execute(
        select(PlaylistTrack).where(PlaylistTrack.playlist_id == sample_playlist.id)
    )
    links = result.scalars().all()
    # Map by track_id
    link_map = {l.track_id: l for l in links}
