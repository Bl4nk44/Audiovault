...
async def test_execute_sync_removal(db_session, sync_manager, admin_user):
    ...
    token = "token123"
    sync_manager._pending_reports[token] = {"watchlist_id": wl.id, "to_remove_items": [{"track_id": str(t.id)}]}
...
async def test_execute_sync_ref_counting(db_session, sync_manager, admin_user):
    ...
    token = "token_ref"
    sync_manager._pending_reports[token] = {"watchlist_id": wl1.id, "to_remove_items": [{"track_id": str(t.id)}]}
