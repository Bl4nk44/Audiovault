...
async def test_refresh_token_errors(db_session):
    ...
    token = jwt.encode(
        {"type": "refresh", "sub": "not-a-uuid"}, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM
    )
    with pytest.raises(HTTPException) as exc:
        await auth_manager.refresh_access_token(token)
    ...
    token = jwt.encode(
        {"type": "refresh", "sub": str(uuid.uuid4())}, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM
    )
    with pytest.raises(HTTPException) as exc:
        await auth_manager.refresh_access_token(token)
