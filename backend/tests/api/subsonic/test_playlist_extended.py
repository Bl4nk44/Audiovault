...
@pytest.mark.asyncio
async def test_update_playlist_add_song(client: AsyncClient, subsonic_auth_params, sample_playlist_ext, db_session, admin_user):
...
@pytest.mark.asyncio
async def test_update_playlist_add_multiple_songs(client: AsyncClient, subsonic_auth_params, sample_playlist_ext, db_session, admin_user):
    ...
    params = {**subsonic_auth_params, "playlistId": str(playlist.id), 
              "songIdToAdd": [str(t.id) for t in new_tracks]}
...
async def test_create_playlist_with_songs(client: AsyncClient, subsonic_auth_params, db_session, admin_user):
    ...
    params = {**subsonic_auth_params, "name": "New With Songs", 
              "songId": [str(t.id) for t in tracks]}
...
@pytest.mark.asyncio
async def test_create_playlist_update_existing(client: AsyncClient, subsonic_auth_params, sample_playlist_ext, db_session, admin_user):
    ...
    params = {**subsonic_auth_params, "playlistId": str(playlist.id), 
              "name": "Updated Name", "songId": str(new_track.id)}
