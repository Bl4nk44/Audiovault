...
async def sample_media(db_session, admin_user):
    ...
    download = Download(
        id=uuid.uuid4(), user_id=admin_user.id, track_id=track.id, status="completed", file_path=file_path
    )
...
async def test_subsonic_get_cover_art_local(client: AsyncClient, subsonic_auth_params, sample_media):
    ...
    with (
        patch("os.path.exists", side_effect=lambda p: "cover.jpg" in p or "test.mp3" in p),
        patch("aiofiles.open", new_callable=MagicMock) as mock_open,
    ):
        mock_file = AsyncMock()
        mock_file.__aenter__.return_value = mock_file
...
async def test_subsonic_get_cover_art_remote(client: AsyncClient, subsonic_auth_params, db_session):
    ...
    with (
        ...
        mock_remote.return_value = Response(
            content=b"remote content", media_type="image/jpeg", headers={"X-Cache": "MISS"}
        )
...
async def test_subsonic_get_cover_art_remote_async(client: AsyncClient, subsonic_auth_params, db_session):
    track = Track(
        id=uuid.uuid4(), title="Remote Art 2", metadata_content={"image_url": "https://i.scdn.co/image/test2"}
    )
    db_session.add(track)
    await db_session.commit()
