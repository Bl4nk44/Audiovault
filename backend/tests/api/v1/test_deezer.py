...
async def test_search_deezer(client, admin_token_headers, mock_deezer_service):
    ...
    response = await client.get("/api/v1/deezer/search", params={"q": "test", "limit": 10}, headers=admin_token_headers)
...
async def test_get_deezer_playlist(client, admin_token_headers, mock_deezer_service):
    ...
    response = await client.get("/api/v1/deezer/playlist/pl1", headers=admin_token_headers)
