...
async def other_user(db_session):
    u = User(id=uuid.uuid4(), username="other", email="other@ex.com", hashed_password=get_password_hash("pw"), is_active=True)
    db_session.add(u)
    await db_session.commit()
    return u
...
async def test_playlist_not_found_scenarios(client, admin_token_headers):
    ...
    assert (await client.put(f"/api/v1/playlists/{pid}", json={"name": "X"}, headers=admin_token_headers)).status_code == 404
    assert (await client.delete(f"/api/v1/playlists/{pid}", headers=admin_token_headers)).status_code == 404
    assert (await client.post(f"/api/v1/playlists/{pid}/tracks", json={"track_ids": [str(uuid.uuid4())]}, headers=admin_token_headers)).status_code == 404
    assert (await client.request("DELETE", f"/api/v1/playlists/{pid}/tracks", json={"track_ids": [str(uuid.uuid4())]}, headers=admin_token_headers)).status_code == 404
...
async def test_get_versions_success(client, admin_token_headers, sample_playlist, db_session):
    v = PlaylistVersion(playlist_id=sample_playlist.id, version_number=1, name="V", change_type="c", tracks_snapshot=[], created_at=__import__('datetime').datetime.now())
    db_session.add(v)
    await db_session.commit()
...
async def test_rollback_success(client, admin_token_headers, sample_playlist, sample_track, db_session):
    v = PlaylistVersion(playlist_id=sample_playlist.id, version_number=1, name="V1", change_type="create", tracks_snapshot=[str(sample_track.id)], created_at=__import__('datetime').datetime.now())
    db_session.add(v)
    await db_session.commit()
    with patch("app.api.v1.playlists.playlist_version_service.rollback_to_version", new_callable=AsyncMock) as mock_rollback:
