...
async def test_get_track_cover_flow(client, admin_token_headers):
    ...
    with patch("app.api.v1.stream._resolve_track_path", new_callable=AsyncMock) as mock_resolve:
        ...
        with patch("app.api.v1.stream.os.path.exists", return_value=False), \
...
def test_extract_art_strategies():
    from app.api.v1.stream import _extract_art_mp4, _extract_art_flac, _extract_art_sync
    import mutagen.mp4
    ...
    # FLAC
    m = MagicMock(); p = MagicMock(); p.data = b"f"; p.mime = "i/f"
    m.pictures = [p]
...
def test_sync_helpers_failures():
    from app.api.v1.stream import _resolve_stream_url_sync, _get_spotify_track_sync
...
@pytest.mark.asyncio
async def test_stream_content_gen():
    from app.api.v1.stream import _stream_content
    class C:
        async def iter_chunked(self, n):
            yield b"x"
    class M:
        def __init__(self): self.content = C()
        async def __aenter__(self): return self
        async def __aexit__(self, *a): pass
    class S:
        def get(self, *a, **k): return M()
        async def __aenter__(self): return self
        async def __aexit__(self, *a): pass
    with patch("aiohttp.ClientSession", return_value=S()):
        assert [x async for x in _stream_content("http://u")] == [b"x"]
