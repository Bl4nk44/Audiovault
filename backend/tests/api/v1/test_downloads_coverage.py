...
async def test_download_all_artist_tracks_invalid_source(client, admin_token_headers):
    response = await client.post(
        "/api/v1/downloads/artist/123/download-all", headers=admin_token_headers, json={"source": "unsupported"}
    )
...
async def test_remove_download_not_found(client, admin_token_headers):
    ...
    response = await client.delete(f"/api/v1/downloads/{dl_id}", headers=admin_token_headers)
...
async def test_bulk_update_invalid_fields(client, admin_token_headers):
    payload = {"download_ids": [str(uuid.uuid4())], "updates": {"forbidden_field": "val"}}
...
async def test_scan_library_access_denied(client, admin_token_headers):
    with patch(
        ...
        response = await client.post(
            "/api/v1/downloads/maintenance/scan-library", headers=admin_token_headers, params={"scan_path": "/root"}
        )
