...
async def test_download_all_artist_tracks_success(
    ...
    admin_token_headers, 
    db_session, 
    spotify_service_mock
):
    # Setup mocks
    artist_id = "artist123"
    spotify_service_mock.get_artist_details.return_value = {"name": "Test Artist"}
    spotify_service_mock.get_artist_albums.return_value = [
...
@pytest.mark.asyncio
async def test_download_album_success(
    client: AsyncClient, 
    admin_token_headers, 
    db_session, 
    spotify_service_mock
...
@pytest.mark.asyncio
async def test_bulk_update_library_items(
    client: AsyncClient, 
    admin_token_headers, 
    db_session, 
    admin_user
    ...
    payload = {
        "download_ids": [str(download.id)],
        "updates": {"artist": "Updated Artist", "genre": "Pop"}
    }
...
@pytest.mark.asyncio
async def test_scan_library_success(
    client: AsyncClient, 
    admin_token_headers
):
    with patch("app.services.library_scanner.library_scanner_service.scan_directory", new_callable=AsyncMock) as mock_scan:
        mock_scan.return_value = {"status": "success", "added": 5}
        response = await client.post(
            "/api/v1/downloads/maintenance/scan-library", 
            headers=admin_token_headers,
            json={"scan_path": "/fake/path"}
        )
        assert response.status_code == 200
